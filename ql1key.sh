...
services:
  ql:
    ...
    container_name: ql
    volumes:
      - ./mnt/mmcblk2p4/data/config:/ql/config
      - ./mnt/mmcblk2p4/data/log:/ql/log
      - ./mnt/mmcblk2p4/data/db:/ql/db
      - ./mnt/mmcblk2p4/data/scripts:/ql/scripts
      - ./mnt/mmcblk2p4/data/repo:/ql/repo
    ports:
      - "0.0.0.0:5700:5700"
    networks:
      - net
...
EOF
    ...
    if [ $? -ne 0 ] ; then
        ...
        sleep 30
        ok "青龙面板已启动，请去浏览器访问http://ip:5700进行初始化并登陆进去，完成后回来继续下一步！"
    fi

else
    error "已有ql名称的容器在运行，不能重复创建！"
	exit 1
fi
}
...
add_repo
sleep 2
ok "已部署完成，2.11.3版本青龙，数据保存路径为$datav，容器名ql，访问地址http://ip:5700"
